...
// From: https://github.com/dotnet/corefx/pull/25410
...
{
    public static partial class DbProviderFactoriesCore
    {
        static DbProviderFactoriesCore()
        {
            RegisterFactory("System.Data.SqlClient", typeof(SqlClientFactory));
        }

        private static class ADP
        {
            public static void CheckArgumentNull(object arg, string argName)
            {
                if (arg == null)
                {
            ...
            internal static void CheckArgumentLength(string value, string parameterName)
            {
                ...
                if (0 == value.Length)
                {
                    throw Argument("SR.ADP_EmptyString, " + parameterName);
                }
            }
        }
